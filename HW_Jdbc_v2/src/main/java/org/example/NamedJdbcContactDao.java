...
public class NamedJdbcContactDao implements ContactDao{
    ...
    public void saveAll(Collection<Contact> contacts) {
        for (Contact contact : contacts) {
            namedJdbcTemplate.update(
                    "INSERT INTO CONTACT(ID, NAME,SURNAME,PHONE_NUMBER,EMAIL) VALUES(:id, :n,:s,:pn,:ea)",
                    new MapSqlParameterSource(Map.of("id",contact.getId(),"n", contact.getName(), "s",contact.getSurname(),"pn",contact.getPhone_number(),"ea",contact.getEmail_address()))
                    );
        }
    }
}
